...
-- 2.1. Individual Customers (Subtype)
...
-- 2.2. Corporate Customers (Subtype)
...
-- 4.1. Fuel Vehicles (Yakıtlı Araçlar - Alt tip)
CREATE TABLE IF NOT EXISTS fuel_vehicles (
    vehicle_id BIGINT PRIMARY KEY REFERENCES vehicles(id) ON DELETE CASCADE,
    fuel_type VARCHAR(50)               -- 'GASOLINE', 'DIESEL', 'LPG'
);

-- 4.2. Electric Vehicles (Elektrikli Araçlar - Alt tip)
...
-- 13. Bank Accounts (Banka Hesapları)
CREATE TABLE IF NOT EXISTS bank_accounts (
    id BIGSERIAL PRIMARY KEY,
    owner_id BIGINT NOT NULL REFERENCES personnel(id) ON DELETE CASCADE,
    bank_name VARCHAR(100),
    account_number VARCHAR(100) UNIQUE,
    balance DOUBLE PRECISION DEFAULT 0 CHECK (balance >= 0)
);

-- 14. Audit Log Tablosu (Trigger için)
...
-- Fuel Vehicles: Yakıt tipi kontrolü
ALTER TABLE fuel_vehicles DROP CONSTRAINT IF EXISTS chk_fuel_type;
ALTER TABLE fuel_vehicles ADD CONSTRAINT chk_fuel_type 
...
-- Electric Vehicles: Batarya kapasitesi pozitif olmalı
ALTER TABLE electric_vehicles DROP CONSTRAINT IF EXISTS chk_battery_capacity;
ALTER TABLE electric_vehicles ADD CONSTRAINT chk_battery_capacity 
...
BEGIN
    IF TG_OP = 'INSERT' THEN
        ...
        VALUES ('vehicles', 'INSERT', NEW.id, row_to_json(NEW), CURRENT_TIMESTAMP);
        RAISE NOTICE 'Yeni araç eklendi: #%', NEW.id;
    ELSIF TG_OP = 'UPDATE' THEN
        INSERT INTO audit_log (table_name, operation, record_id, old_data, new_data, changed_at)
        VALUES ('vehicles', 'UPDATE', NEW.id, row_to_json(OLD), row_to_json(NEW), CURRENT_TIMESTAMP);
        RAISE NOTICE 'Araç güncellendi: #%', NEW.id;
    ELSIF TG_OP = 'DELETE' THEN
        INSERT INTO audit_log (table_name, operation, record_id, old_data, changed_at)
        VALUES ('vehicles', 'DELETE', OLD.id, row_to_json(OLD), CURRENT_TIMESTAMP);
        RAISE NOTICE 'Araç silindi: #%', OLD.id;
    END IF;
    RETURN NULL;
END;
$$ LANGUAGE plpgsql;
...
-- -------------------------------------------------------
-- TRIGGER 6: Ödeme yapıldığında para transferi
-- Açıklama: Ödeme tablosuna kayıt düştüğünde,
-- Müşteriden düşüp Galericiye ekler.
-- -------------------------------------------------------
...
DECLARE
    ...
    v_dealer_id BIGINT;
    v_customer_balance DOUBLE PRECISION;
BEGIN
    -- Satış ödemesi ise
    IF NEW.sale_id IS NOT NULL THEN
        -- İlgili satışı bul (müşteri ve ilan->galerici)
        SELECT s.customer_id, l.dealer_id 
        INTO v_customer_id, v_dealer_id
        FROM sales s
        JOIN listings l ON s.listing_id = l.id
        WHERE s.id = NEW.sale_id;
        
        -- Müşteri bakiyesi kontrol
        SELECT balance INTO v_customer_balance FROM bank_accounts WHERE owner_id = v_customer_id;
        
        IF v_customer_balance < NEW.amount THEN
            RAISE EXCEPTION 'Yetersiz bakiye! Mevcut: %, Istenen: %', v_customer_balance, NEW.amount;
        END IF;

        -- Transfer
        UPDATE bank_accounts SET balance = balance - NEW.amount WHERE owner_id = v_customer_id;
        UPDATE bank_accounts SET balance = balance + NEW.amount WHERE owner_id = v_dealer_id;
        
        RAISE NOTICE 'Transfer basarili: % -> % (Miktar: %)', v_customer_id, v_dealer_id, NEW.amount;
        
    -- Kiralama ödemesi ise
    ELSIF NEW.rental_id IS NOT NULL THEN
        SELECT r.customer_id, l.dealer_id 
        INTO v_customer_id, v_dealer_id
        FROM rentals r
        ...
        RAISE NOTICE 'Kiralama odemesi basarili: % -> % (Miktar: %)', v_customer_id, v_dealer_id, NEW.amount;
    END IF;
...
DROP TRIGGER IF EXISTS trg_process_money_transfer ON payments;
...
INSERT INTO customers (id, phone, customer_type) VALUES
(4, '+90 532 123 4567', 'INDIVIDUAL'),
(5, '+90 542 987 6543', 'INDIVIDUAL'),
(6, '+90 505 111 2233', 'INDIVIDUAL'),
(7, '+90 533 555 6677', 'INDIVIDUAL'),
(8, '+90 544 999 8877', 'INDIVIDUAL'),
(9, '+90 506 222 3344', 'INDIVIDUAL'),
(10, '+90 535 666 7788', 'INDIVIDUAL'),
(11, '+90 507 333 4455', 'INDIVIDUAL')
ON CONFLICT (id) DO NOTHING;

-- 2. Insert into individual_customers
INSERT INTO individual_customers (customer_id, national_id, birth_date) VALUES
(4, '12345678901', '1985-03-15'),
(5, '98765432109', '1990-07-22'),
(6, '11122233344', '1988-11-30'),
(7, '55566677788', '1995-05-18'),
(8, '99988877766', '1978-09-25'),
(9, '22233344455', '1992-01-12'),
(10, '66677788899', '1987-06-08'),
(11, '33344455566', '1993-12-03')
ON CONFLICT (customer_id) DO NOTHING;

-- Customers (Kurumsal)
-- 1. Insert into customers base table
INSERT INTO customers (id, phone, customer_type) VALUES
(12, '+90 212 555 1234', 'CORPORATE'),
(13, '+90 216 444 5678', 'CORPORATE'),
(14, '+90 312 333 9012', 'CORPORATE'),
(15, '+90 232 222 3456', 'CORPORATE')
ON CONFLICT (id) DO NOTHING;

-- 2. Insert into corporate_customers
INSERT INTO corporate_customers (customer_id, company_name, tax_number) VALUES
(12, 'Tech Corp Ltd.', '1234567890'),
(13, 'ABC Holding A.S.', '2345678901'),
(14, 'Logistik Tasimacilik A.S.', '3456789012'),
(15, 'Ege Turizm Ltd. Sti.', '4567890123')
ON CONFLICT (customer_id) DO NOTHING;
...
(1, 'Tesla', 'Model 3', 2023, 'White', '34 ABC 123', 15000, '5YJ3E1EA5NF000001', 'ELECTRIC'),
(2, 'Tesla', 'Model Y', 2024, 'Red', '34 TES 001', 5000, '5YJ3E1EA5NF000002', 'ELECTRIC'),
(3, 'Audi', 'Q4 e-tron', 2023, 'Blue', '07 MNO 345', 12000, 'WAUZZZF48NA012345', 'ELECTRIC'),
(4, 'BMW', 'iX3', 2023, 'Black', '06 BMW 333', 18000, 'WBAXXXXXXX000003', 'ELECTRIC'),
(5, 'Mercedes', 'EQC', 2022, 'Silver', '34 EQC 500', 25000, 'WDCXXXXXXX000004', 'ELECTRIC'),
-- Fuel Vehicles (6-13)
(6, 'BMW', '320i', 2022, 'Black', '06 DEF 456', 32000, 'WBA5R1C50NAJ12345', 'FUEL'),
(7, 'Mercedes', 'E 200', 2021, 'Gray', '34 JKL 012', 48000, 'WDD2130401A123456', 'FUEL'),
(8, 'Audi', 'A4', 2022, 'White', '35 AUD 100', 28000, 'WAUXXXXXXX000005', 'FUEL'),
(9, 'Volkswagen', 'Passat', 2021, 'Blue', '34 VW 2021', 55000, 'WVWXXXXXXX000006', 'FUEL'),
(10, 'Ford', 'Focus', 2020, 'Red', '06 FRD 200', 65000, 'WFXXXXXXXX000007', 'FUEL'),
(11, 'Renault', 'Megane', 2021, 'White', '35 RNT 100', 42000, 'VFXXXXXXXX000008', 'FUEL'),
(12, 'Opel', 'Astra', 2020, 'Gray', '34 OPL 999', 58000, 'WOXXXXXXXX000009', 'FUEL'),
(13, 'Fiat', 'Egea', 2022, 'White', '34 FIA 500', 35000, 'ZFXXXXXXXX000010', 'FUEL'),
-- Hybrid Vehicles (14-18)
(14, 'Toyota', 'Corolla Hybrid', 2024, 'Silver', '35 GHI 789', 5000, 'JTDKN3DU2A0123456', 'HYBRID'),
(15, 'Toyota', 'RAV4 Hybrid', 2023, 'Green', '07 RAV 400', 15000, 'JTXXXXXXXX000011', 'HYBRID'),
(16, 'Honda', 'CR-V Hybrid', 2023, 'Blue', '34 HND 200', 12000, 'JHLXXXXXXX000012', 'HYBRID'),
(17, 'Lexus', 'ES 300h', 2022, 'Black', '34 LEX 300', 22000, 'JTXXXXXXXX000013', 'HYBRID'),
(18, 'Hyundai', 'Tucson Hybrid', 2024, 'White', '06 TUC 100', 8000, 'KMXXXXXXXX000014', 'HYBRID'),
-- Luxury Vehicles (19-20)
(19, 'Porsche', 'Taycan', 2023, 'Black', '34 POR 911', 10000, 'WPXXXXXXXX000015', 'ELECTRIC'),
(20, 'Range Rover', 'Sport', 2022, 'Green', '34 RR 2022', 30000, 'SALXXXXXXX000016', 'FUEL')
ON CONFLICT (id) DO NOTHING;

-- INSERT INTO electric_vehicles
INSERT INTO electric_vehicles (vehicle_id, battery_capacity, range_km) VALUES
(1, 82.0, 580),
(2, 75.0, 533),
(3, 77.0, 520),
(4, 80.0, 460),
...
INSERT INTO fuel_vehicles (vehicle_id, fuel_type) VALUES
(6, 'GASOLINE'),
(7, 'DIESEL'),
(8, 'GASOLINE'),
(9, 'DIESEL'),
...
INSERT INTO personnel_addresses (id, personnel_id, city, district, neighborhood, street, building_no, postal_code) VALUES
...
SELECT setval('personnel_addresses_id_seq', (SELECT COALESCE(MAX(id), 1) FROM personnel_addresses));
...
-- =====================================================
-- BANK ACCOUNTS (15 hesap)
-- =====================================================

INSERT INTO bank_accounts (id, owner_id, bank_name, account_number, balance) VALUES
-- Dealers (Zengin)
(1, 1, 'Ziraat Bankasi', 'TR111111', 500000.0),
(2, 2, 'Garanti BBVA', 'TR222222', 250000.0),
(3, 3, 'Is Bankasi', 'TR333333', 1000000.0),
-- Customers (Bireysel - Alım Gücü Var)
(4, 4, 'Akbank', 'TR444444', 200000.0),
(5, 5, 'Vakifbank', 'TR555555', 150000.0),
(6, 6, 'Ziraat Bankasi', 'TR666666', 300000.0),
(7, 7, 'Garanti BBVA', 'TR777777', 80000.0), -- Düşük bakiye testi için
(8, 8, 'Yapi Kredi', 'TR888888', 250000.0),
(9, 9, 'Finansbank', 'TR999999', 400000.0),
(10, 10, 'Is Bankasi', 'TR101010', 500000.0),
(11, 11, 'Ziraat Bankasi', 'TR111112', 120000.0),
-- Customers (Kurumsal - Yüksek Bakiye)
(12, 12, 'Halkbank', 'TR121212', 5000000.0),
(13, 13, 'Akbank', 'TR131313', 10000000.0),
(14, 14, 'Vakifbank', 'TR141414', 2000000.0),
(15, 15, 'Garanti BBVA', 'TR151515', 750000.0)
ON CONFLICT (id) DO NOTHING;
...
SELECT 'Addresses (Adresler): ' || COUNT(*) as info FROM personnel_addresses;
...
SELECT 'Bank Accounts (Hesaplar): ' || COUNT(*) as info FROM bank_accounts;
SELECT '========================================' as info;
